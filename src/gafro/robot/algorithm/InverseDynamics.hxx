...
namespace gafro::algorithm
{
                                                           ...
                                                           const T &gravity)
    {
        ...
        ForwardIteration<T, 0>::compute(kinematic_chain->getActuatedJoints(), kinematic_chain->getBodies(), position, velocity, acceleration, frames_,
                                        twists_, twists_dt_, gravity_);

        BackwardIteration<T, 0>::compute(kinematic_chain->getBodies(), frames_, twists_, twists_dt_, wrench_, torque_);
                                                                  ...
                                                                  const E3i<T> &gravity)
    {
        ...
        twist_dt[j] = twist_dt[j - 1].transform(frame[j])                                //
                      + Scalar<T>(velocity[j]) * bodies[j]->getAxis().commute(twist[j])  //
                      + Scalar<T>(acceleration[j]) * bodies[j]->getAxis();

        ForwardIteration<V, j + 1>::compute(joints, bodies, position, velocity, acceleration, bodies[j]->getCenterOfMass(), frame, twist, twist_dt,
                                            gravity);
    }
    ...
    struct InverseDynamics<T, dof>::ForwardIteration<V, 0>
    {
                            ...
                            const E3i<T> &gravity)
        {
            ...
            twist_dt[0] = gravity                                                            //
                          + Scalar<T>(velocity[0]) * bodies[0]->getAxis().commute(twist[0])  //
                          + Scalar<T>(acceleration[0]) * bodies[0]->getAxis();

            ForwardIteration<V, 1>::compute(joints, bodies, position, velocity, acceleration, bodies[0]->getCenterOfMass(), frame, twist, twist_dt,
                                            gravity);
        }
    };
